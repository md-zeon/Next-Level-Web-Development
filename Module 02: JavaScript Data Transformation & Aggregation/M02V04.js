...
console.log(
	"Products A:",
	productsA.map((p) => p.name),
);
console.log(
	"Products B:",
	productsB.map((p) => p.name),
);
...
console.log(
	"Students A ∩ B ∩ C:",
	intersectionMultiple(studentsA, studentsB, studentsC),
); // []
...
console.log(
	"Students A ∪ B ∪ C:",
	unionMultiple(studentsA, studentsB, studentsC),
);
...
const fromResult = Array.from(
	new Set(largeArray1.filter((x) => largeArray2.includes(x))),
);
...
console.log(
	"• intersection() with includes() is O(n*m) - avoid for large arrays",
);
...
console.log("\n=== Lesson Complete ===");
console.log(
	"Array cross matching and Array.from() are powerful tools for data manipulation!",
);
