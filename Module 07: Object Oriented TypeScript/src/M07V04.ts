...
function handlePet(pet: Animal): void {
...
function printShapeInfo(shape: Shape): void {
	...
	if (shape instanceof Rectangle) {
		...
		console.log(
			`Type: Triangle with base ${shape.base} and height ${shape.height}`,
		);
...
function processData(data: unknown): void {
	if (data instanceof Array) {
		...
		console.log(
			`Date: ${data.toLocaleDateString()} ${data.toLocaleTimeString()}`,
		);
		...
		console.log(
			`Set with ${data.size} elements: {${Array.from(data).join(", ")}}`,
		);
...
const personObj: Person = { name: "John", age: 30 };
...
class DigitalProduct extends Product {
	constructor(name: string, price: number, public fileSize: number) {
		super(name, price);
	}
...
class Furniture extends PhysicalProduct {
	constructor(
		...
		price: number,
		weight: number,
		public requiresAssembly: boolean,
	) {
	...
	getDeliveryMethod(): string {
		return `Shipping ${
			this.requiresAssembly ? "(assembly required)" : "(ready to use)"
		}`;
	}
}
...
class GasolineCar extends Vehicle {
	constructor(brand: string, model: string, public engineSize: number) {
		super(brand, model);
	}
