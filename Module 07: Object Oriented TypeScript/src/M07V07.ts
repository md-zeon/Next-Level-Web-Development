...
class User {
  ...
  static fromObject(obj: { id: number; name: string; email: string; role?: string }): User {
...
class AppConfig {
  ...
  static readonly SUPPORTED_FORMATS: readonly string[] = ["json", "xml", "csv"];
...
class DatabaseConfig {
  ...
  private static _port: number = 5432;
  private static _database: string = "myapp";
  private static _connectionString: string | null = null;
...
class Animal {
  ...
  static getTotalCount(): string {
    return `Total animals: ${Animal.totalAnimals}`;
  }

  static getSpeciesList(): string {
...
class ApiClient {
  ...
  static getBaseInfo(): string {
    return `API ${ApiClient.apiVersion} at ${ApiClient.baseURL}`;
  }
}

class UserApiClient extends ApiClient {
  static apiVersion: string = "v2"; // Shadows parent static property
